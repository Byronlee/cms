...
describe Asynces::CommentsController do
  ...
  describe "POST 'create'" do
    it 'returns http success' do
      post :create, post_id: comment.commentable.id, comment: attributes_for(:comment)
      expect(response).to be_success
      expect(assigns(:comment).is_long?).to be_false
      expect(response).to render_template('asynces/comments/_comment')
    end

    it 'should be long comment if length > 140' do
      post :create, post_id: comment.commentable, comment: { content: 'A' * 150 }
      expect(assigns(:comment).is_long?).to be_true
    end

    it 'should be invalid if length > 3000' do
      post :create, post_id: comment.commentable, comment: { content: 'A' * 3001 }
